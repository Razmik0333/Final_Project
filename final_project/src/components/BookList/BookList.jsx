...
// import './BookList.css';
// import { currentCategorySelector, currentPageSelector, changeCurrencyes }
...
//   return 0;
// }));
// const getSortedByPriceArray = (data) =>
// (data.sort((a, b) => a.pageCount * 10 - b.pageCount * 10));
...
// const arrayStartPage = (data, first, last) =>
// data.filter((_, pos) => pos >= first && pos <= last);
...
//         {
//           filters.map((filter) => <NavLink onClick={handleFilter}
// className="filter-items active-filter" data-filter={`${filter.alias}`}
// to={`/filter/${filter.alias}`}>{filter.arm}</NavLink>)
//         }
//       </nav>
...
//  !== undefined ? item.thumbnailUrl : noImg} data-id={item.isbn} alt="" />
...
//           <ul className="pages">
//             {
//               data.map((_, page) => ((page > 0 && page <= pageCount)
//  ? <li className="page"><NavLink onClick={handlePage} data-page={page}
//  to={`/page/${page}`}>{page}</NavLink></li> : false))
//             }
//           </ul>
//         </nav>
//       </main>
